...
void resolve_traverse(
    ...
    int failsafe{ 0 };
    while (!all_stacks_are_empty(job_stacks) && failsafe < 100) {
        failsafe++;
        log_stream << std::endl << "================== *** Time " << time_pos << " *** ==================" << std::endl;
        ...
        initial_node.next_task_vec_idx = 0; // We start with the first task in the stack whatever happens
        ...
        std::vector<std::tuple<int, int>> candidate_tasks{}; // first integer is the tardiness score, second integer is the task index
        for (auto& [j_idx, task_stack] : job_stacks) {
            // Insert the first task in line for the job if it exists and if the processing time of its predecessor is over
            if (task_stack.empty() || time_pos < next_time_persue_job[j_idx]) {
                continue;
        ...
        log_stream << "Candidate tasks for processing (in order): { ";
        ...
        while (!node_queue.empty()) {
            ...
            std::tuple<int, int>& processed_task = candidate_tasks[node.next_task_vec_idx];
            ...
            for (auto& m_idx : inst.tasks[t_idx].machines) {
                ...
                for (auto& o_idx : inst.tasks[t_idx].compatibility[m_idx]) {
                    ...
                    // copy the parent node, and update fields afterwards
                    ExplorationNode child_node = node;
                    child_node.available_operators.reset(o_idx);
                    child_node.available_machines.reset(m_idx);
